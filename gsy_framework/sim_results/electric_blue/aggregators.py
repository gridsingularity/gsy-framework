...
class EBEnergyProfileAggregator(BaseAggregator):
    ...
    def __call__(self, collected_raw_data: Dict):
        ...
        for trade in collected_raw_data["trades"]:
            ...
            energy = trade["energy"]
            # SELLER
            current_result[seller_id]["matched_sell_orders_kWh"] += energy
            current_result[seller_id]["all_sell_orders_kWh"] += energy
            # BUYER
            current_result[buyer_id]["matched_buy_orders_kWh"] += energy
            current_result[buyer_id]["all_buy_orders_kWh"] += energy

        for bid in collected_raw_data["bids"]:
            buyer_id = bid["buyer_id"]
            energy = bid["energy"]
            # BUYER
            current_result[buyer_id]["open_buy_orders_kWh"] += energy
            current_result[buyer_id]["all_buy_orders_kWh"] += energy
        ...
        for offer in collected_raw_data["offers"]:
            ...
            energy = offer["energy"]
            # SELLER
            current_result[seller_id]["open_sell_orders_kWh"] += energy
            current_result[seller_id]["all_sell_orders_kWh"] += energy
