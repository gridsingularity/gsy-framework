...
class UserProfileReader:
    ...
    def _str_to_datetime(self, time_string, time_format) -> DateTime:
        """
        Converts time_string into a pendulum (DateTime) object that either takes the global
        start date or the provided one, dependent on the time_format
        :return: DateTime
        """
    ...
    def default_profile_dict(self, val=None, current_timestamp=None) -> Dict[DateTime, int]:
        """
        ...
        return {
            ...
            for time_slot in generate_market_slot_list(
                start_timestamp=current_timestamp, profile_length_days=self._profile_length_days
            )
        }

    def _remove_header(self, profile_dict: Dict) -> Dict:
        """
    ...
    def _eval_single_format(self, time_dict, time_format):
    ...
    def _eval_time_format(self, time_dict: Dict) -> str:
        """
        ...
        for time_format in [
            ...
            if self._eval_single_format(time_dict, time_format):
                return time_format
        ...
        with open(path, encoding="utf-8") as csv_file:
            ...
            for row in csv_rows:
                ...
                try:
                    ...
                    pass
        time_format = self._eval_time_format(profile_data)
        return dict(
            (self._str_to_datetime(time_str, time_format), value)
            for time_str, value in profile_data.items()
        )

    def _interpolate_profile_values_to_slot(self, profile_data_W, slot_length):
        ...
        slot_time_list = list(
            range(
                input_time_seconds_list[0], input_time_seconds_list[-1], slot_length.in_seconds()
            )
        )
    ...
    def _calculate_energy_from_power_profile(
        self, profile_data_W: Dict[DateTime, float], slot_length: duration
    ) -> Dict[DateTime, float]:
        # pylint: disable=invalid-name
        """
        ...
        avg_power_kW, slot_time_list = self._interpolate_profile_values_to_slot(
            profile_data_W, slot_length
        )
    ...
    def _fill_gaps_in_profile(self, input_profile: Dict = None, out_profile: Dict = None) -> Dict:
        """
        Fills time steps, where no value is provided, with the value of the last available time
        step.
        ...
        :param out_profile: dict with the same format as the input_profile that can be used
                            to provide a default zero-value profile dict with the expected
                            timestamps that need to be populated in the profile
    ...
    def _read_from_different_sources_todict(
        ...
        if os.path.isfile(str(input_profile)):
            # input is csv file
            profile = self._read_csv(input_profile)
            ...
            if isinstance(input_profile, str):
                ...
                profile = self._remove_header(profile)
                time_format = self._eval_time_format(profile)
                profile = {
                    self._str_to_datetime(key, time_format): val for key, val in profile.items()
                }
                ...
                input_profile = self._remove_header(input_profile)
                # Remove filename from profile
                input_profile.pop("filename", None)
                time_format = self._eval_time_format(input_profile)
                profile = {
                    self._str_to_datetime(key, time_format): val
            ...
            profile = self.default_profile_dict(
                val=input_profile, current_timestamp=current_timestamp
            )
    ...
    def _hour_time_str(self, hour: float, minute: float) -> str:
        return f"{hour:02}:{minute:02}"

    def _copy_profile_to_multiple_days(
        self, in_profile: Dict, current_timestamp: Optional[datetime] = None
    ) -> Dict:
        daytime_dict = dict(
            (self._hour_time_str(time.hour, time.minute), time) for time in in_profile.keys()
        )
        out_profile = {}
        for slot_time in generate_market_slot_list(start_timestamp=current_timestamp):
            if slot_time not in out_profile:
                time_key = self._hour_time_str(slot_time.hour, slot_time.minute)
    ...
    def read_arbitrary_profile(
        ...
        if input_profile in [{}, None]:
            return {}
        profile = self._read_from_different_sources_todict(
        ...
        if (
            ...
            profile = self._copy_profile_to_multiple_days(
                profile, current_timestamp=current_timestamp
            )
        ...
        try:
            ...
            if profile_type is InputProfileTypes.CARBON_RATIO_G_KWH:
                return profile
            zero_value_slot_profile = self.default_profile_dict(
                current_timestamp=current_timestamp
            )
            filled_profile = self._fill_gaps_in_profile(profile, zero_value_slot_profile)
            if profile_type in [
    ...
    def _generate_slot_based_zero_values_dict_from_profile(self, profile, slot_length_mins=15):
        profile_time_list = list(profile.keys())
    ...
    def read_profile_without_config(
        self, input_profile: Dict, slot_length_mins=15
    ) -> Dict[DateTime, float]:
        """Read profile without using a configuration."""
        profile = self._read_from_different_sources_todict(input_profile)
        if profile is not None:
            slot_based_profile = self._generate_slot_based_zero_values_dict_from_profile(
                profile, slot_length_mins
            )
            filled_profile = self._fill_gaps_in_profile(profile, slot_based_profile)
            profile_values, slots = self._interpolate_profile_values_to_slot(
    ...
    def _generate_time_slots(
        self, slot_length: timedelta, sim_duration: timedelta, start_date: datetime
    ) -> Generator:
        return (
    ...
    def _get_from_profile(self, profile: Dict[datetime, float], key: datetime) -> float:
        try:
            return profile[key]
    ...
    def resample_hourly_energy_profile(
        ...
        if slot_length_minutes < 60:
            ...
            out_dict = {}
            for time_slot in self._generate_time_slots(slot_length, sim_duration, start_date):
                ...
                out_dict[time_slot] = (
                    self._get_from_profile(input_profile, hour_time_slot) / scaling_factor
                )
            return out_dict
        if slot_length_minutes > 60:
