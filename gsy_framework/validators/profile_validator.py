...
class ProfileValidator:
    ...
    def __init__(
        ...
        slot_length: Optional[timedelta] = None,
    ):
    ...
    def _validate_start_end_date(self):
        assert self._profile_start_time <= self.start_time, (
            f"Profile start time {self._profile_start_time.format(DATE_TIME_FORMAT)} "
            f"is not consistent with the simulation "
            f"start time {self.start_time.format(DATE_TIME_FORMAT)}."
        )
        if (
            self._profile_end_time - self._profile_start_time
            == duration(days=1) - self.slot_length
        ):
        ...
        assert self._profile_end_time >= self.end_time, (
            f"Profile end time {self._profile_end_time.format(DATE_TIME_FORMAT)} "
            f"is not consistent with the simulation "
            f"end time {self.end_time.format(DATE_TIME_FORMAT)}."
        )
    ...
    def _validate_slot_length(self):
        ...
        for time_slot in list(self.profile.keys())[1:]:
            assert first_time.add(seconds=int(self.slot_length.total_seconds())) == time_slot, (
                f"Profile timestamp {time_slot.format(DATE_TIME_FORMAT)} "
                f"does not follow the slot length "
                f"{self.slot_length.total_seconds() / 60} minutes "
                f"(previous time_slot: {first_time.format(DATE_TIME_FORMAT)})"
            )
