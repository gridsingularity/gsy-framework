...
class HeatPumpValidator(BaseValidator):
    ...
    def validate(cls, **kwargs):
        ...
        cls._check_range(
            name="tank_volume_l",
            value=kwargs["tank_parameters"].tank_volume_L,
    ...
    def _validate_profiles(cls, **kwargs):
        ...
        if (
            kwargs.get("external_temp_C_profile") is None
            and kwargs.get("external_temp_C_profile_uuid") is None
        ):
            raise GSyDeviceException(
                {"misconfiguration": ["external_temp_C_profile should be provided."]}
            )
    ...
    def _validate_temp(cls, **kwargs):
        ...
        min_temp_c = kwargs["tank_parameters"].min_temp_C
        max_temp_c = kwargs["tank_parameters"].max_temp_C
        initial_temp_c = kwargs["tank_parameters"].initial_temp_C
