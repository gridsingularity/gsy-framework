...
Ranges = namedtuple('Ranges', ('min', 'max'))
...
class ConstSettings:
    class GeneralSettings:
        ...
        RATE_DECREASE_PER_UPDATE_RANGE = Ranges(0, 1000)
        RATE_CHANGE_PER_UPDATE = Ranges(0, 1000)

        INITIAL_RATE_OPTIONS = [1, 2, 3]

        NUM_CLONES_RANGE = Ranges(0, 100)
        MIN_NUM_TICKS = 10
        MAX_SLOT_LENGTH_M = 60
        MIN_TICK_LENGTH_S = 1

    class CommercialProducerSettings:
        ENERGY_RATE_RANGE = Ranges(0, 10000)
    ...
    class StorageSettings:
        ...
        BREAK_EVEN_RANGE = Ranges(0, 10000)
        # Max battery capacity in kWh.
        CAPACITY = 1.2
        CAPACITY_RANGE = Ranges(0.0001, 2000000)
        # Maximum battery power for supply/demand, in Watts.
        MAX_ABS_POWER = 5
        MAX_ABS_POWER_RANGE = Ranges(0.0001, 2000000)
        # Energy buy-range, storage never buys outside this limit.
        # Unit is ct/kWh.
        BUYING_RANGE = Ranges(0, 24.9)
        INITIAL_BUYING_RANGE = Ranges(0, 10000)
        FINAL_BUYING_RANGE = Ranges(0, 10000)
        # Energy sell-range, storage never sell outside this limit.
        # Unit is ct/kWh.
        SELLING_RANGE = Ranges(30, 25)
        INITIAL_SELLING_RANGE = Ranges(0, 10000)
        FINAL_SELLING_RANGE = Ranges(0, 10000)
    ...
    class LoadSettings:
        AVG_POWER_RANGE = Ranges(0, sys.maxsize)
        HOURS_RANGE = Ranges(0, 24)
        # Min load energy rate, in ct/kWh
        INITIAL_BUYING_RATE = 0
        INITIAL_BUYING_RATE_RANGE = Ranges(0, 10000)
        # Max load energy rate, in ct/kWh
        FINAL_BUYING_RATE = 35
        FINAL_BUYING_RATE_RANGE = Ranges(0, 10000)

    class PVSettings:
        PANEL_COUNT_RANGE = Ranges(1, 10000)
        MIN_SELL_RATE_RANGE = Ranges(0, 10000)
        INITIAL_RATE_RANGE = Ranges(0, 10000)
        MAX_PANEL_OUTPUT_W_RANGE = Ranges(0, sys.maxsize)
        ...
        CLOUD_COVERAGE_RANGE = Ranges(0, 3)
    ...
    class IAASettings:
        ...
        FEE_PERCENTAGE = 0
        FEE_PERCENTAGE_RANGE = Ranges(0, 100)
        FEE_CONSTANT = 0
        FEE_CONSTANT_RANGE = Ranges(0, sys.maxsize)
        # Market type option
