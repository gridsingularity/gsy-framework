...
def test_log_market_progression(caplog):
    ...
    gsy_framework.client_connections.utils.logging.info = MagicMock()
    log_market_progression(message)
    assert gsy_framework.client_connections.utils.logging.info.call_count == 0
    ...
    log_market_progression(message)
    assert gsy_framework.client_connections.utils.logging.info.called_once_with(
        f"\n\n{tabulate([table_data, ], headers=headers, tablefmt='fancy_grid')}\n\n"
    )
    message["slot_completion"] = "9%"
    log_market_progression(message)
    assert gsy_framework.client_connections.utils.logging.info.called_once()
