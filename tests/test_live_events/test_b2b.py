...
class TestB2BLiveEvents:
    ...
    @pytest.mark.parametrize("validate_method, arguments", [
        (LiveEventArgsValidator(lambda x, y: None).validate_start_trading_event_args,
         ["start_time", "end_time", "market_type", "capacity_percent", "energy_rate"]),
        (LiveEventArgsValidator(lambda x, y: None).validate_stop_trading_event_args,
    ...
    def test_validate_capacity_percent():
        ...
        assert validator.validate_start_trading_event_args(args_copy) is False
        args_copy["capacity_percent"] = 123.0
        assert validator.validate_start_trading_event_args(args_copy) is False
        args_copy["capacity_percent"] = 99.0
        assert validator.validate_start_trading_event_args(args_copy) is True
    ...
    def test_validate_start_end_time_on_stop_trading(time_parameter_name):
        ...
        assert validator.validate_stop_trading_event_args(args_copy) is True
        args_copy[time_parameter_name] = "01-01-2022:13:23"
        assert validator.validate_stop_trading_event_args(args_copy) is False
